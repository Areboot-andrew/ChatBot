...
def build_system_prompt(settings: BotSetting, qa_facts: List[QaPair] = None, rag_docs: List[str] = None, include_grounding: bool = True) -> str:
    ...
    if include_grounding:
        tpl_evaluation_rules = meta.get("tpl_evaluation_rules", "--- [ABSOLUTE RULE: HALLUCINATION BAN] ---\n1. If the question is about technical specs, compatibility, availability, prices or business facts - use ONLY the verified context blocks above.\n2. If there is NO direct answer in the provided context - you are forbidden from inventing it from memory.\n3. Ask only the minimum useful clarification. Do not reflexively ask for an exact model, photo or link. Ask for exact model/photo/link only when it is truly required for a model-specific part, compatibility, exact price, warranty/identity check, or when the item type is unclear.\n4. For service/repair flows, if the item type is clear but the problem is missing, ask what happened or what is not working. Do not name a failed component without diagnostic evidence.\n5. NO assumptions about compatibility. Either confirmation is in context, or say it needs confirmation.")
