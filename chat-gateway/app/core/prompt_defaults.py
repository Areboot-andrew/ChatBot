...
ROUTE_PROMPTS = {
    ...
    "catalog": {
        ...
        "result_validation_prompt": "Compare complete phrases. The category and row must explicitly describe the same item/device type and requested product/service. Reject a row from another category even if it shares words such as screen, matrix, battery, board, bouquet or composition. Never assign an unfamiliar item to a broad category using general world knowledge: a returned list of categories is not proof that the requested item belongs to one of them. A category match can prove broad availability only when the requested generic device type is explicitly named by that category or a matching row; it cannot prove a specific price. A price is valid only from a matching internal row.",
